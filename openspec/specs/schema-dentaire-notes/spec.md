# schema-dentaire-notes

## Purpose

Faire figurer sur une note d'honoraires les **dents concernées** par les actes facturés, à la
fois en **texte agrégé** (numéros FDI dédupliqués et triés, nombre de dents) et sous forme d'un
**schéma odontogramme anatomique numéroté** mettant visuellement en évidence les dents traitées —
pour une note mono-acte comme pour une note multi-actes (un seul schéma agrégeant l'ensemble des
actes retenus). Le schéma est **rendu côté serveur, sans navigateur**, et inséré dans le `.docx`
en remplacement d'une balise document `<ODONTOGRAMME>`. Tout est **calculé au rendu**, jamais
stocké : aucune migration de schéma, aucune écriture sur les actes, aucun effet sur la dette.
