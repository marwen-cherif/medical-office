...
def _import_rows(conn, rows: list[_Row], dry_run: bool) -> ImportSummary:
    ...
    for line_no, raw_id, libelle, raw_prix, raw_code, raw_cat, raw_actif in rows:
        if (
            not libelle
            and raw_prix is None
            and raw_code is None
            and raw_cat is None
            and raw_id is None
        ):
        ...
        try:
            ...
            summary.errors.append(
                f"Ligne {line_no} ({libelle or '?'}) : {exc}, ignoree."
            )
            summary.skipped += 1
            continue
...
def export_actes(path: Path, include_inactive: bool = False, conn=None) -> int:
    ...
    if own:
        conn = connect()
    try:
        actes = repo.list_actes(conn, actifs_seulement=not include_inactive, limit=None)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.fichier:
        parser.error("indiquez un fichier .xlsx a importer (ou --export / --modele).")
