...
def _build_devmode(printer_name: str, paper: str | None, color: str | None):
    ...
    if paper_dm is None and color_dm is None:
        if paper or color:
            log.warning(
                "Reglage ignore (valeur inconnue): paper=%r color=%r", paper, color
            )
        return None
    try:
        ...
        try:
            ...
            if devmode is None:
                log.warning(
                    "Imprimante %r sans DEVMODE : format/couleur non appliques",
                    printer_name,
                )
                return None
            ...
            try:
                ...
                log.warning(
                    "DocumentProperties a echoue (%s) : on garde le DEVMODE brut", exc
                )
        finally:
            win32print.ClosePrinter(handle)
        log.info(
            "DEVMODE construit: paper=%s (DM=%s) color=%s (DM=%s)",
            paper,
            paper_dm,
            color,
            color_dm,
        ...
        log.warning(
            "Construction du DEVMODE impossible pour %r (repli sur defaut)",
            printer_name,
            exc_info=True,
        )
        return None
...
def _print_images(
    ...
    try:
        if devmode is not None:
            before = (
                hdc.GetDeviceCaps(_PHYSICALWIDTH),
                hdc.GetDeviceCaps(_PHYSICALHEIGHT),
            )
            try:
                if not win32gui.ResetDC(hdc.GetSafeHdc(), devmode):
                    log.warning(
                        "ResetDC a renvoye 0 : DEVMODE non applique (format par defaut)"
                    )
            except Exception:  # noqa: BLE001  (pilote refusant le DEVMODE : on continue)
                log.warning(
                    "ResetDC a echoue : impression au DEVMODE par defaut", exc_info=True
                )
            after = (
                hdc.GetDeviceCaps(_PHYSICALWIDTH),
                hdc.GetDeviceCaps(_PHYSICALHEIGHT),
        ...
        offset = (
            hdc.GetDeviceCaps(_PHYSICALOFFSETX),
            hdc.GetDeviceCaps(_PHYSICALOFFSETY),
        )
...
def print_file(
    ...
    *,
    ...
    path = Path(path)
    _get_logger().info(
        "Impression: fichier=%s imprimante=%r paper=%r color=%r",
        path.name,
        printer_name,
        paper,
        color,
...
def print_test_page(
    ...
    *,
    ...
    try:
        ...
        if cabinet_name and not cabinet_name.startswith("Cabinet"):
            cabinet_name = f"Cabinet {cabinet_name}"
    except Exception:
        cabinet_name = "Cabinet Médical"

    draw.text((120, 140), "Test d'impression", fill="black", font=title_font)
    draw.text((120, 260), cabinet_name, fill="black", font=body_font)
