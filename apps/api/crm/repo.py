...
def count_patients(
    conn: sqlite3.Connection, search: str = "", filtre: str = "tous"
) -> int:
    where, params = _patient_filter_clause(search, filtre)
...
def _row_to_document(row: sqlite3.Row) -> Document:
    return Document(
        ...
        mailjet_opened_at=(
            row["mailjet_opened_at"] if "mailjet_opened_at" in row.keys() else None
        ),
        mailjet_clicked_at=(
...
def create_document(conn: sqlite3.Connection, d: Document) -> Document:
    cur = conn.execute(
        ...
        (
            ...
            d.date_envoi,
            d.email,
            d.mailjet_message_id,
            d.mailjet_status,
            d.date_refresh_status,
            d.message_erreur,
            d.variables,
            d.categorie,
...
def _batch_patient_clause(
    ...
    if date_to.strip():
        ...
        params.append(date_to.strip())
    clauses = [
        "EXISTS (SELECT 1 FROM documents d WHERE " + " AND ".join(sub_clauses) + ")"
    ]
    if search.strip():
        needle = f"%{slugify(search)}%"
        clauses.append(
...
def mark_paiement_encaisse(
    ...
    if pa is None:
        return
    if pa.reste > 1e-9:
        add_paiement_reglement(
            conn, paiement_id, pa.reste, mode=mode, date_reglement=when
        )
...
def list_template_fields(
    conn: sqlite3.Connection, template_name: str
) -> list[TemplateField]:
    rows = conn.execute(
...
def replace_template_fields(
    ...
    conn.execute(
        "DELETE FROM template_fields WHERE template_name = ?", (template_name,)
    )
...
def list_categories(conn: sqlite3.Connection) -> list[Category]:
    ...
    rows = conn.execute("SELECT * FROM categories ORDER BY sort_order, nom").fetchall()
...
def list_acte_categories(
    ...
    """
    sql = (
        "SELECT DISTINCT categorie FROM actes "
...
def create_job(
    ...
    cur = conn.execute(
        "INSERT INTO jobs (kind, doc_type, statut, total, params) VALUES (?,?,?,?,?)",
...
def count_jobs(conn: sqlite3.Connection, date_from: str = "", date_to: str = "") -> int:
    where, params = _jobs_period_clause(date_from, date_to)
...
def count_factures_for_prestataire(
    conn: sqlite3.Connection, prestataire_id: int
) -> int:
    row = conn.execute(
...
def create_depense(
    ...
    *,
    ...
    if montant is None or montant <= 0:
        raise ValueError(
            "Le montant d'une depense doit etre strictement superieur a 0."
        )
...
def count_depenses_for_prestataire(
    conn: sqlite3.Connection, prestataire_id: int
) -> int:
    row = conn.execute(
...
def total_regle_periode(
    ...
    where = (" WHERE " + " AND ".join(clauses)) if clauses else ""
    row = conn.execute(
        f"SELECT COALESCE(SUM(montant), 0) AS total FROM depense_reglements{where}",
        params,
    ).fetchone()
    return float(row["total"] or 0)
...
_FDI_VALIDES: frozenset[str] = frozenset(
    n
    for quadrants in (DENTS_PERMANENTES, DENTS_TEMPORAIRES)
    for nums in quadrants.values()
    for n in nums
)
...
def plan_totaux(conn: sqlite3.Connection, plan_id: int) -> tuple[float, float, float]:
...
def _creances_union(
    ...
    note_where = " AND ".join(
        [
            ...
            "(pa.montant - pa.montant_regle) > 1e-9",
        ]
        + note_clauses
    )
    acte_where = " AND ".join(
        ...
        for r in notes:
            ...
            out.append(
                Creance(
                    ...
                    montant=float(pa.montant or 0),
                    reste=pa.reste,
                    date=pa.date_echeance,
                    statut=pa.statut,
                )
            )
    return out
    ...
    *,
    ...
    for c in creances_patient(conn, patient_id, include_notes=include_notes):
        ...
        if pay <= 1e-9:
            continue
        if c.nature == "acte":
            add_prestation_reglement(
                conn, c.source_id, pay, mode=mode, date_reglement=when
            )
    ...
    return {
        "alloue": float(montant) - remaining,
        "reste": max(0.0, remaining),
        "lignes": lignes,
    }
