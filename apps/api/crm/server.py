...
@app.post(
    "/api/templates/{name}/rename", response_model=TemplateOut, tags=["templates"]
)
...
@app.post(
    "/api/templates/{name}/open-in-word", response_model=OkOut, tags=["templates"]
)
def templates_open_in_word(name: str) -> OkOut:
    ...
    try:
        ...
        raise ApiError(
            ERR_WORD_UNAVAILABLE,
            f"Impossible d'ouvrir le modèle dans Word : {exc}",
            status=500,
        )
    return OkOut()
...
@app.get(
    ...
    response_model=PlaceholdersOut,
    tags=["templates"],
)
def templates_placeholders(name: str) -> PlaceholdersOut:
    ...
    return PlaceholdersOut(
        document_tags=doc_tags, line_tags=line_tags, auto_tags=auto, custom_tags=custom
    )


@app.get(
    "/api/templates/{name}/fields", response_model=list[FieldOut], tags=["templates"]
)
def templates_fields(name: str) -> list[FieldOut]:
    with db() as conn:
        return [
            FieldOut(
                tag=f.tag, label=f.label, type=f.type, default_value=f.default_value
            )
...
@app.get(
    ...
    response_model=CategoryValueOut,
    tags=["templates"],
)
...
@app.get(
    "/api/mail-templates", response_model=list[MailTemplateOut], tags=["mailTemplates"]
)
...
@app.put(
    "/api/mail-templates/{tid}", response_model=MailTemplateOut, tags=["mailTemplates"]
)
def mail_templates_update(tid: int, body: MailTemplateIn) -> MailTemplateOut:
    with db() as conn:
        repo.update_mail_template(
            conn,
            repo.MailTemplate(
                id=tid,
                name=body.name,
...
@app.post(
    "/api/mail-templates/{tid}/default", response_model=OkOut, tags=["mailTemplates"]
)
...
def printers_test(body: PrinterTestIn) -> JobAcceptedOut:
    ...
    if name not in printing.list_printers():
        raise ApiError(
            ERR_PRINTER_NOT_FOUND, f"Imprimante introuvable : {name}", status=404
        )
...
@app.get(
    "/api/settings/print/{doc_type}", response_model=PrintConfigOut, tags=["settings"]
)
...
def actes_update(acte_id: int, body: ActeIn) -> ActeOut:
    try:
        with db() as conn:
            ...
            if existing is None:
                raise ApiError(
                    ERR_NOT_FOUND, f"Acte introuvable : {acte_id}", status=404
                )
            repo.update_acte(
                conn,
                repo.Acte(
...
def actes_export(body: ActeExportIn) -> ActeExportOut:
    ...
    try:
        ...
        OSError
    ) as exc:  # dossier non inscriptible, fichier verrouille (ouvert dans Excel)
        raise ApiError(
            ERR_XLSX, f"Impossible d'ecrire le fichier d'export : {exc}", status=400
        )
    except Exception as exc:  # noqa: BLE001
        raise _err_from_engine(exc)
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    parser.add_argument(
        "--host", default="127.0.0.1", help="Interface d'ecoute (loopback par defaut)."
    )
    parser.add_argument(
        "--port", type=int, default=0, help="Port (0 = ephemere, choisi par l'OS)."
    )
    parser.add_argument(
        "--token", default=None, help="Jeton de session (genere si absent)."
    )
    parser.add_argument(
        "--reload",
        action="store_true",
