...
def _to_prestataire(body: PrestataireIn, pid: Optional[int] = None) -> repo.Prestataire:
    return repo.Prestataire(
        id=pid,
        nom=body.nom.strip(),
        prenom=(body.prenom or "").strip(),
        email=body.email or None,
        telephone=body.telephone or None,
...
def prestataires_list(
    search: str = "", limit: Optional[int] = 20, offset: int = 0
) -> PrestataireListOut:
...
@router.post(
    "/prestataires", response_model=PrestataireOut, responses={409: {"model": MatchOut}}
)
def prestataires_create(body: PrestataireIn) -> PrestataireOut:
    if not (body.nom or "").strip():
        raise core.ApiError(
            core.ERR_VALIDATION,
            "Le nom / la raison sociale est obligatoire.",
            status=400,
        )
    try:
        with core.db() as conn:
...
def factures_list(
    prestataire_id: int, limit: Optional[int] = 20, offset: int = 0
) -> FactureListOut:
    with core.db() as conn:
    ...
    try:
        ...
        with core.db() as conn:
            ...
            fac = generator.import_facture(
                conn, p, tmp, montant=montant, libelle=libelle
            )
            repo.log_audit(
                conn,
                "facture_importee",
    ...
    try:
        with core.db() as conn:
            dep = repo.create_depense(
                ...
                body.prestataire_id,
                body.montant,
                montant_regle=body.montant_regle,
                motif=body.motif,
            ...
            repo.log_audit(
                conn, "depense_creee", {"depense_id": dep.id, "montant": dep.montant}
            )
...
def depense_reglement(depense_id: int, body: ReglementDepenseIn) -> DepenseOut:
    try:
        with core.db() as conn:
            ...
            if existing is None:
                raise core.ApiError(
                    core.ERR_NOT_FOUND, "Dépense introuvable.", status=404
                )
            dep = repo.add_depense_reglement(
                conn,
                depense_id,
                body.versement,
                mode=body.mode,
...
@router.get(
    "/prestataires/{prestataire_id}/reglements",
    response_model=PrestataireReglementListOut,
)
...
@router.get(
    "/depenses/{depense_id}/reglements", response_model=list[DepenseReglementOut]
)
