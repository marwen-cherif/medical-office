...
def jobs_list(
    date_from: str = "", date_to: str = "", limit: Optional[int] = 20, offset: int = 0
) -> JobListOut:
    with core.db() as conn:
        items = repo.list_jobs(
            conn, limit=limit, offset=offset, date_from=date_from, date_to=date_to
        )
...
def job_detail(job_id: int) -> JobDetailOut:
    with core.db() as conn:
        ...
        if job is None:
            raise core.ApiError(
                core.ERR_NOT_FOUND, f"Job introuvable : {job_id}", status=404
            )
                        ...
                        try:
                            if kind == "generation":
                                ...
                                generator.send_document(
                                    conn, d, config, template_id=mailjet_template_id
                                )
                            repo.add_job_item(
                                conn, repo_job_id, d.patient_id, "ok", document_id=d.id
                            )
...
def documents_batch(body: BatchIn) -> BatchAcceptedOut:
    ...
    if not ids:
        raise core.ApiError(
            core.ERR_VALIDATION, "Aucun document sélectionné.", status=400
        )
    ...
    with core.db() as conn:
        ...
        params = json.dumps(
            {"mode": "documents", "mailjet_template_id": mailjet_template_id},
            ensure_ascii=False,
        )
...
@router.post(
    "/jobs/{job_id}/relaunch", response_model=BatchAcceptedOut, status_code=202
)
def job_relaunch(job_id: int) -> BatchAcceptedOut:
    ...
    with core.db() as conn:
        ...
        if not ids:
            raise core.ApiError(
                core.ERR_VALIDATION, "Aucune erreur à relancer pour ce job.", status=400
            )
        ...
        if job.kind == "envoi":
            ...
            mailjet_template_id = default.mailjet_template_id if default else None
        params = json.dumps(
            {"mode": "documents", "relance_de": job_id}, ensure_ascii=False
        )
