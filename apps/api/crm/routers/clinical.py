...
def paiement_out(pa: repo.Paiement) -> PaiementOut:
    return PaiementOut(
        ...
        montant=pa.montant,
        montant_regle=pa.montant_regle,
        reste=pa.reste,
        statut=pa.statut,
        mode=pa.mode,
        date_echeance=pa.date_echeance,
        date_encaissement=pa.date_encaissement,
        notes=pa.notes,
    )
...
def patient_clinical(patient_id: int) -> ClinicalOut:
    with core.db() as conn:
        notes = [
            pa
            for pa in repo.list_paiements(conn, patient_id)
        ...
        for pl in plans:
            ...
            groups.append(
                PlanGroupOut(
                    plan=plan_out(pl),
                    totaux=TotauxOut(du=du, encaisse=enc, reste=reste),
...
def patient_encaissements(
    patient_id: int, limit: Optional[int] = 20, offset: int = 0
) -> EncaissementsOut:
    with core.db() as conn:
        items = repo.list_encaissements_patient(
            conn, patient_id, limit=limit, offset=offset
        )
    ...
    return EncaissementsOut(
        ...
        total=total,
        solde=TotauxOut(du=du, encaisse=enc, reste=reste),
    )
...
def patient_audit(
    patient_id: int, limit: int = 200, offset: int = 0, category: AuditCategory = "tous"
) -> list[AuditOut]:
    with core.db() as conn:
        rows = repo.list_audit_patient(
            conn, patient_id, limit=limit, offset=offset, category=category
        )
    return [
        AuditOut(ts=ts, action=action, detail=repo.parse_audit_detail(detail))
...
def plan_create(patient_id: int, body: PlanIn) -> PlanOut:
    try:
        with core.db() as conn:
            pl = repo.create_plan(
                conn, patient_id, body.titre.strip(), body.notes or None
            )
            repo.log_audit(
                conn,
                "plan_cree",
                {"plan_id": pl.id, "titre": pl.titre},
...
def plan_update(plan_id: int, body: PlanIn) -> PlanOut:
    try:
        with core.db() as conn:
            ...
            if existing is None:
                raise core.ApiError(
                    core.ERR_NOT_FOUND, f"Plan introuvable : {plan_id}", status=404
                )
...
def plan_delete(plan_id: int) -> core.OkOut:
    with core.db() as conn:
        ...
        repo.log_audit(
            conn, "plan_supprime", {"plan_id": plan_id}, patient_id=existing.patient_id
        )
    return core.OkOut()
    ...
    try:
        with core.db() as conn:
            ...
            if existing is None:
                raise core.ApiError(
                    core.ERR_NOT_FOUND,
                    f"Acte introuvable : {prestation_id}",
                    status=404,
                )
    ...
    try:
        with core.db() as conn:
            ...
            repo.log_audit(
                ...
                {
                    "montant": body.montant,
                    "alloue": res.get("alloue"),
                    "mode": body.mode,
                },
                patient_id=patient_id,
            )
        return CascadeOut(
            alloue=res.get("alloue", 0.0),
            reste=res.get("reste", 0.0),
...
def paiement_reglement(paiement_id: int, body: ReglementIn) -> PaiementOut:
    ...
    try:
        with core.db() as conn:
            ...
            if existing is None:
                raise core.ApiError(
                    core.ERR_NOT_FOUND, f"Note introuvable : {paiement_id}", status=404
                )
...
def paiement_encaisser(paiement_id: int, body: EncaisserIn) -> core.OkOut:
    ...
    with core.db() as conn:
        repo.mark_paiement_encaisse(
            conn, paiement_id, when=body.date_encaissement, mode=body.mode
        )
