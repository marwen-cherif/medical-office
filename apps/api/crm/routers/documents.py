...
def document_out(d: repo.Document) -> DocumentOut:
    ...
    return DocumentOut(
        ...
        email=d.email,
        mailjet_status=d.mailjet_status,
...
def _resolve_fields(conn, template: templates.Template) -> list[repo.TemplateField]:
    ...
    if saved:
        ...
        out = [
            repo.TemplateField(template.name, tag, _humanize(tag), _guess_type(tag), "")
    ...
    if needs_dents and not any(f.tag.upper() == "DENTS" for f in out):
        out.append(
            repo.TemplateField(template.name, "DENTS", "Dents concernées", "text", "")
        )
    return out
...
def patient_documents(
    patient_id: int, limit: Optional[int] = 20, offset: int = 0
) -> DocumentListOut:
...
def documents_filtered(
    ...
    with core.db() as conn:
        rows = repo.list_documents_filtered(
            conn,
            search=search,
            statut=statut,
            limit=limit,
        ...
        total = repo.count_documents_filtered(
            conn, search=search, statut=statut, date_from=date_from, date_to=date_to
        )
    return DocumentRowsOut(
        items=[
            DocumentRow(document=document_out(d), patient=patient_out(p))
            for d, p in rows
        ],
        total=total,
    )
...
def _acte_line(
    p: repo.Prestation, checked: bool = True, montant_note: Optional[float] = None
) -> GenActeLine:
    return GenActeLine(
        id=p.id,
        libelle=p.libelle,
...
def generation_form(
    ...
    source_prestation_id: Optional[int] = None,
) -> GenFormOut:
    ...
    with core.db() as conn:
        ...
        if tpl is None:
            raise core.ApiError(
                core.ERR_NOT_FOUND, f"Modèle introuvable : {template}", status=404
            )
        draft_vars: dict = {}
        out_fmt = "jpg"
        ...
        if multi:
            ...
            saved_lines = (
                [l for l in saved if isinstance(l, dict) and l.get("source") == "acte"]
                if isinstance(saved, list)
                else None
            )
            saved_ids = (
                {l.get("prestation_id") for l in saved_lines}
                if saved_lines is not None
                else None
            )
            # Montant de note edite par acte (restitue a la reouverture du brouillon).
            saved_montants = {
                l.get("prestation_id"): l.get("montant")
            ...
            for pl in plans:
                ...
                groups.append(
                    GenPlanGroup(
                        titre=pl.titre,
                        prestations=[
                            _acte_line(x, checked(x.id), montant_note(x.id))
                            for x in pres
                        ],
                    )
                )
            actes = GenActesSource(
                isoles=[
                    _acte_line(x, checked(x.id), montant_note(x.id)) for x in isoles
                ],
                plans=groups,
            )
            return GenFormOut(
        ...
        src_pres = (
            repo.get_prestation(conn, source_prestation_id)
            if source_prestation_id and not draft_vars
            else None
        )
        fields: list[GenFieldOut] = []
    ...
    for na in body.new_actes:
        ...
        if na.prestation_id:
            repo.update_prestation(
                conn,
                na.prestation_id,
                libelle=na.libelle.strip(),
                montant=na.montant,
                plan_id=None,
                acte_id=na.acte_id,
                date_acte=na.date_acte,
                dents=na.dents,
                note=na.note,
            )
...
def _persist_draft(conn, patient_id: int, body: DraftIn) -> repo.Document:
    ...
    if tpl is None:
        raise core.ApiError(
            core.ERR_NOT_FOUND, f"Modèle introuvable : {body.template}", status=404
        )
    ...
    if body.document_id is not None:
        ...
        if draft is None:
            raise core.ApiError(
                core.ERR_NOT_FOUND, "Brouillon introuvable.", status=404
            )
        doc = generator.update_draft(
            conn, draft, variables=variables, output_format=body.output_format
        )
        repo.log_audit(
            conn,
            "brouillon_modifie",
    ...
    doc = generator.save_draft(
        conn, patient, tpl, variables=variables, output_format=body.output_format
    )
    repo.log_audit(
        conn,
        "brouillon_cree",
...
def _check_printer(conn) -> str:
    ...
    if available and printer not in available:
        raise core.ApiError(
            core.ERR_PRINTER_NOT_FOUND,
            f"Imprimante « {printer} » indisponible.",
            status=404,
        )
    return printer
...
@router.post(
    "/patients/{patient_id}/documents/generate",
...
def generate(patient_id: int, body: GenerateIn) -> core.JobAcceptedOut:
    ...
    def task(report):
        ...
        try:
            ...
            repo.log_audit(
                conn,
                "note_honoraires_generee" if body.is_note else "document_genere",
...
def render(document_id: int) -> core.JobAcceptedOut:
    def task(report):
        ...
        try:
            ...
            if d is None:
                raise core.ApiError(
                    core.ERR_NOT_FOUND, "Document introuvable.", status=404
                )
    ...
    def task(report):
        ...
        try:
            ...
            try:
                ...
                raise core.ApiError(
                    core.ERR_PRINT_FAILED, f"Échec de l'impression : {exc}"
                )
            repo.log_audit(
                conn,
                "document_imprime",
...
@router.post(
    "/documents/{document_id}/send", response_model=core.JobAcceptedOut, status_code=202
)
...
@router.post(
    "/documents/{document_id}/refresh-status",
    response_model=core.JobAcceptedOut,
    status_code=202,
)
...
def send_whatsapp(
    document_id: int, body: Optional[SendWhatsAppIn] = None
) -> core.JobAcceptedOut:
    with core.db() as conn:
        d = repo.get_document(conn, document_id)
        if d is None:
        ...
        if not settings.get("whatsapp_phone_number_id") or not settings.get(
            "whatsapp_access_token"
        ):
            raise core.ApiError(
                "CONFIG_ERROR",
    ...
    def task(report):
        ...
        try:
            ...
            generator.send_document_whatsapp(
                conn, doc, pat, whatsapp_settings, target_phone=target_phone
            )
