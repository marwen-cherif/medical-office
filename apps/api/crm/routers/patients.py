...
def _to_patient(body: PatientIn, patient_id: Optional[int] = None) -> repo.Patient:
    return repo.Patient(
        ...
        date_naissance=body.date_naissance or None,
...
def patients_list(
    search: str = "", filtre: str = "tous", limit: Optional[int] = 20, offset: int = 0
) -> PatientListOut:
    with core.db() as conn:
        items = repo.list_patients(
            conn, search=search, filtre=filtre, limit=limit, offset=offset
        )
...
def patient_detail(patient_id: int) -> PatientDetailOut:
    with core.db() as conn:
        ...
        if p is None:
            raise core.ApiError(
                core.ERR_NOT_FOUND, f"Patient introuvable : {patient_id}", status=404
            )
...
@router.post(
    "/patients", response_model=PatientOut, responses={409: {"model": DuplicateWarning}}
)
def patients_create(body: PatientIn) -> PatientOut:
    if not (body.nom or "").strip() or not (body.prenom or "").strip():
        raise core.ApiError(
            core.ERR_VALIDATION, "Le nom et le prénom sont obligatoires.", status=400
        )
    try:
        with core.db() as conn:
            if not body.force:
            ...
            repo.log_audit(conn, "fiche_creee", {"display": p.display}, patient_id=p.id)
    ...
    try:
        with core.db() as conn:
            ...
            if existing is None:
                raise core.ApiError(
                    core.ERR_NOT_FOUND,
                    f"Patient introuvable : {patient_id}",
                    status=404,
                )
            p = _to_patient(body, patient_id)
            changed = repo.update_patient(conn, p)
            repo.log_audit(
                conn, "fiche_modifiee", {"champs": changed}, patient_id=patient_id
            )
            p = repo.get_patient(conn, patient_id)
        return patient_out(p)
    except core.ApiError:
