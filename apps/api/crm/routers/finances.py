        ...
        if statut == "en_attente":
            ...
            items = [
                FinanceRow(
                    ...
                    patient_display=c.patient.display,
                    libelle=c.libelle,
                    montant=c.montant,
                    reste=c.reste,
                    date=c.date,
            ...
            summary = FinanceSummary(
                ...
                total=repo.total_creances(
                    conn, search=search, date_from=date_from, date_to=date_to
                ),
            )
        else:
            total = repo.count_paiements(
                conn, search=search, statut=statut, date_from=date_from, date_to=date_to
            )
            rows = repo.list_paiements_filtered(
                conn,
                search=search,
                statut=statut,
                limit=limit,
            ...
            items = [
                FinanceRow(
                    ...
                    source_id=pa.id,
                    patient_id=pt.id,
                    patient_display=pt.display,
                    libelle=pa.notes or "Paiement",
                    montant=pa.montant,
                    reste=pa.reste,
                    date=pa.date_encaissement or pa.date_echeance,
                    statut=pa.statut,
                    mode=pa.mode,
                )
                for pa, pt in rows
...
def finances_depenses(
    ...
    with core.db() as conn:
        total = repo.count_depenses(
            conn, search=search, statut=statut, date_from=date_from, date_to=date_to
        )
        rows = repo.list_depenses_filtered(
            conn,
            search=search,
            statut=statut,
            limit=limit,
        ...
        du, regle, reste = repo.total_depenses(
            conn, search=search, statut=statut, date_from=date_from, date_to=date_to
        )
    items = [
        DepenseRow(
            depense=depense_out(dep),
