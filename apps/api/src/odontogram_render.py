...
class _Chart:
    ...
    def _tooth_center(
        self, tooth: dict, reflect_x: bool, reflect_y: bool
    ) -> tuple[float, float]:
...
def render_svg(dents: Iterable[str]) -> Optional[str]:
    ...
    for c in charts:
        if captioned:
            label = (
                "Denture permanente" if c.denture == "adulte" else "Denture temporaire"
            )
...
def render_png(
    dents: Iterable[str], out_path: Optional[Path] = None, *, zoom: float = 3.0
) -> Optional[Path]:
