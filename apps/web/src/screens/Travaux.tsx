  ...
  return (
    <div className="mx-auto max-w-6xl p-8">
      <header className="mb-6">
        ...
        <p className="mt-1 text-sm text-muted">Documents générés et jobs de traitement par lot.</p>
      </header>
      ...
      <TableCell>
        <div className="flex items-center justify-end gap-1">
          {needsGeneration ? (
            ...
            d.has_file && (
              <>
                <Button variant="ghost" size="icon" title="Ouvrir" onClick={() => onOpen(d.id)}>
                  <FolderOpen className="size-4" />
                </Button>
                <Button variant="ghost" size="icon" title="Imprimer" onClick={() => onPrint(d.id)}>
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="truncate font-semibold text-ink">
                    #{job.id} · {isGen ? 'Génération' : 'Envoi email'} — {humanize(job.doc_type)}
                  </span>
                  <Badge variant={st.variant}>{st.label}</Badge>
                </div>
