...
function Donut({ a, b, colorA, colorB }: { a: number; b: number; colorA: string; colorB: string }) {
  const r = 42;
  const c = 2 * Math.PI * r;
  const total = a + b;
  ...
  return (
    <div className="mx-auto max-w-6xl p-8">
      <header className="mb-6 flex flex-wrap items-end justify-between gap-4">
        ...
        <DateRangeFilter from={range.from} to={range.to} onChange={(r) => setRange(r)} />
      </header>
      ...
      {kpis && (
        <div className="space-y-6">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <KpiTile label="Encours à recouvrer" value={fmtDevise(kpis.encours)} icon={Wallet} />
            <KpiTile
              label="Solde net"
            ...
            <KpiTile
              ...
              icon={Banknote}
            />
            <KpiTile label="Documents créés" value={String(kpis.nb_documents)} icon={FileStack} />
          ...
          <div className="grid gap-3 lg:grid-cols-2">
            <BalanceCard
              title="Trésorerie"
              donut={<Donut a={kpis.ca_encaisse} b={kpis.encours} colorA={GREEN} colorB={AMBER} />}
            >
              <LegendRow
                color={GREEN}
                label="Encaissé"
            ...
            <BalanceCard
              ...
              donut={
                <Donut a={kpis.ca_encaisse} b={kpis.depenses_reglees} colorA={NAVY} colorB={TEAL} />
              }
            >
              <LegendRow
                color={NAVY}
            ...
            <div className="rounded-[var(--radius)] border border-line bg-white p-5">
              ...
              {docs.length === 0 ? (
                ...
                <ul className="divide-y divide-line">
                  {docs.map((d) => (
                    <li key={d.type} className="flex items-center justify-between py-2 text-sm">
                  ...
                  {activity.map((a, i) => (
                    <li key={i} className="py-2">
                      <div className="flex items-baseline justify-between gap-3">
                        ...
                        <span className="shrink-0 text-xs text-muted">{isoToFrDateTime(a.ts)}</span>
                      </div>
