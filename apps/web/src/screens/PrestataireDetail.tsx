...
export function PrestataireDetail() {
  ...
  if (detailQ.isError) {
    return <p className="mx-auto max-w-5xl p-8 text-sm text-red">{humanizeError(detailQ.error)}</p>;
  }
  ...
  return (
    <div className="mx-auto max-w-6xl p-6">
      ...
      {edit && <PrestataireFormDialog target={edit} onClose={() => setEdit(null)} />}
    </div>
  );
}
...
function IdentityCard({ p }: { p: Prestataire }) {
  return (
    <div className="rounded-[var(--radius)] border border-line bg-white p-5">
      ...
      <Row label="Adresse" value={p.adresse} multiline />
      <Row label="Notes" value={p.notes} multiline />
    </div>
  );
}
...
function FacturesSection({ id }: { id: number }) {
  ...
  return (
    <section className="mt-8">
      ...
      {facturesQ.isError && <p className="text-sm text-red">{humanizeError(facturesQ.error)}</p>}
...
function DepensesSection({ id }: { id: number }) {
  ...
  return (
    <section className="mt-8">
      ...
      {depensesQ.isError && <p className="text-sm text-red">{humanizeError(depensesQ.error)}</p>}
