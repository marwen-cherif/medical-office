  ...
  return (
    <div className="flex flex-wrap gap-1">
      {list.map((d) => (
        <span
          key={d}
          className="rounded bg-teal/20 px-1.5 py-0.5 text-[11px] font-medium text-teal-dark"
        >
          {d}
        </span>
      ))}
...
export function PlansActesTab({
  ...
  const [presDialog, setPresDialog] = useState<{
    target: Prestation | 'new';
    planId?: number | null;
  } | null>(null);
  ...
  if (clinical.isError)
    return <p className="pt-4 text-sm text-red">{humanizeError(clinical.error)}</p>;
    ...
    <div className="space-y-5 pt-4">
              ...
              >
                ...
                <div className="flex-1 text-sm">
                  ...
                  <div className="text-xs text-muted">
                    {partiel && (
                      <span>
                        Réglé {fmtDevise(n.montant_regle)} / {fmtDevise(n.montant)} ·{' '}
                      </span>
                    )}
      ...
      <section className="space-y-1 rounded-[var(--radius)] border border-line bg-white p-4">
        <div className="mb-1 flex items-center justify-between">
          ...
          <Button
            variant="ghost"
            size="sm"
            onClick={() => setPresDialog({ target: 'new', planId: null })}
          >
      ...
      {data.plans.map((g) => {
        ...
        return (
          <section
            key={g.plan.id}
            className="space-y-1 rounded-[var(--radius)] border border-line bg-white p-4"
          >
            <div className="flex items-center gap-2">
