  ...
  return (
    <div className="space-y-1">
      ...
      {lines.map((l) => {
        ...
        const differs =
          isChecked && Math.abs(noteMontantValue(l, montants[l.id]) - l.montant) > 1e-9;
        return (
...
export function GenerateDialog({
  ...
  const totals = useMemo(() => {
    ...
    for (const c of cards)
      if (c.libelle.trim()) du += Number((c.montant || '0').replace(',', '.')) || 0;
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      variant="secondary"
                      size="sm"
                      onClick={() => setCards((c) => [...c, emptyActe()])}
                    >
                  ...
                  <div key={f.tag} className="space-y-2">
                    ...
                    {f.tag.toUpperCase() === 'DENTS' ? (
                      ...
                      <Odontogramme
                        ...
                        onChange={(dents) => setMono((m) => ({ ...m, [f.tag]: dents.join(', ') }))}
                        defaultDenture={defaultDenture}
                      />
                    ) : f.type === 'paragraph' ? (
                      <Textarea
