...
export function useSetTemplateFields() {
  ...
  return useMutation({
    ...
    onSuccess: (_d, v) => qc.invalidateQueries({ queryKey: keys.templateFields(v.name) }),
  });
}
...
export function useDeleteMailTemplate() {
  ...
  return useMutation({
    mutationFn: async (id: number) =>
      unwrap(await client.DELETE('/api/mail-templates/{tid}', { params: { path: { tid: id } } })),
    onSuccess: () => qc.invalidateQueries({ queryKey: keys.mailTemplates }),
  });
}
...
export function useSetPrintConfig() {
  ...
  return useMutation({
    ...
    onSuccess: (_d, v) => qc.invalidateQueries({ queryKey: ['settings', 'print', v.docType] }),
  });
}
...
export function useActes(search: string, includeInactive: boolean, categorie?: string) {
  return useQuery({
...
export function useWhatsAppSettings() {
  return useQuery({
    queryKey: keys.whatsappSettings,
    queryFn: async () => unwrap(await client.GET('/api/settings/whatsapp')),
  });
}

export function useSetWhatsAppSettings() {
  const qc = useQueryClient();
  return useMutation({
