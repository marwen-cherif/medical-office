...
export function useImportFacture(prestataireId: number) {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const resp = await fetch(`${backend.baseUrl}/api/prestataires/${prestataireId}/factures`, {
        method: 'POST',
...
export function useDeleteFacture(prestataireId: number) {
  ...
  return useMutation({
    mutationFn: async (id: number) =>
      unwrap(
        await client.DELETE('/api/factures/{facture_id}', { params: { path: { facture_id: id } } })
      ),
    onSuccess: () => invalidate(qc, prestataireId),
  });
}
...
export function useDeleteDepense(prestataireId?: number) {
  ...
  return useMutation({
    mutationFn: async (id: number) =>
      unwrap(
        await client.DELETE('/api/depenses/{depense_id}', { params: { path: { depense_id: id } } })
      ),
    onSuccess: () => invalidate(qc, prestataireId),
  });
}
