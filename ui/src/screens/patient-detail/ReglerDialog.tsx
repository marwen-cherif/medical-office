...
import { DEVISE_SYMBOLE, fmtDevise, isoToFr, MODE_OPTIONS, montantInput, todayIso } from "@/lib/format";
...
export function ReglerDialog({
  ...
  useEffect(() => {
    if (open && creances.data) setMontant(montantInput(totalReste));
              ...
              <div className="space-y-1.5">
                {preview.map(({ c, alloue }) => (
                  <div key={`${c.nature}-${c.source_id}`} className="flex items-center gap-2 text-sm">
                    ...
                    <Montant value={c.reste} tone="muted" />
